...
all_data = train_data[:N].numpy()
all_target = train_target[:N].numpy()
...
all_data = np.reshape(all_data, (N, -1))
d, idx = distance_scipy_spatial(all_data, k=K)
print("Shape of idx is ", idx.shape)
col_list = []
row_list = []
correct_connect_sum = 0
for i in range(N):
    col_list += idx[i].tolist()
    row_list += [i] * len(idx[i])

    label_i = all_target[i]
    connected_labels = all_target[idx[i]]

    correct_connect_sum += np.sum(connected_labels == label_i)

print("The ratio of correctly connected nodes is ", correct_connect_sum / (N * K))
...
# np.save(data_path + 'train_target.npy', train_target)
# np.save(data_path + 'val_target.npy', valid_target)
# np.save(data_path + 'test_target.npy', test_target)
